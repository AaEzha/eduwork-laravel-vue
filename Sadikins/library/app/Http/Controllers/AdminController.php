...
class AdminController extends Controller
{
    ...
    public function query()
    {
        ...
        $data12 = Transaction::select('name', 'phone_number', 'address', 'date_start', 'date_end', 'books.isbn', 'book_transaction.qty')
            ->join('members', 'transactions.member_id', 'members.id')
            ->join('book_transaction', 'transactions.id', 'book_transaction.transaction_id')
            ->join('books', 'book_transaction.book_id', 'books.id')
            ->havingRaw('book_transaction.qty > 1')
            ->orderBy('book_transaction.qty', 'DESC')
            ->get();
        $data13 = Transaction::select('name', 'phone_number', 'address', 'date_start', 'date_end', 'books.isbn', 'book_transaction.qty', 'title', 'price', DB::raw('(book_transaction.qty * price) as total_price'))
            ->join('members', 'transactions.member_id', 'members.id')
            ->join('book_transaction', 'transactions.id', 'book_transaction.transaction_id')
            ->join('books', 'book_transaction.book_id', 'books.id')
            ->orderBy('book_transaction.qty', 'DESC')
        ...
        $data14 = Transaction::select('members.name', 'members.phone_number', 'members.address', 'date_start', 'date_end', 'books.isbn', 'book_transaction.qty', 'title', 'publishers.name', 'authors.name', 'catalogs.name')
            ->join('members', 'members.id', 'transactions.member_id')
            ->join('book_transaction', 'book_transaction.transaction_id', 'transactions.id')
            ->join('books', 'book_transaction.book_id', 'books.id')
